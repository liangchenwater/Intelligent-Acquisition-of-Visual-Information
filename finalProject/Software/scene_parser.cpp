...
void SceneParser::parsePerspectiveCamera() {
  ...
  float anglex_degrees = readFloat();
  float anglex_radians = DegreesToRadians(anglex_degrees);
  getToken(token); assert (!strcmp(token, "cx"));
  float cx = readFloat();
  getToken(token); assert (!strcmp(token, "cy"));
  float cy = readFloat();
  getToken(token); assert (!strcmp(token, "}"));
  camera = new PerspectiveCamera(center,direction,up,anglex_radians,cx,cy);
}
